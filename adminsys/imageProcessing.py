...
def customizeMockup(path, savepath):
    ...
    img = cv.imread(path)
    mockup1 = cv.imread(f"{settings.MEDIA_ROOT}/lap-back.png")

    mockup2 = cv.imread(f"{settings.MEDIA_ROOT}/lap-keyboard.png")
    ...
    print("/"+savepath+"_b.png")
    cv.imwrite(
        savepath+"_b.png", img_out1)
    cv.imwrite(
        savepath+"_k.png", img_out2)
    # mask1 = np.reshape(mask1, [w,l,3])
    return img_out1, img_out2


def customizeMockup_console(path, savepath):
